...
class JSONParser(Parser):
    ...
    def load(self, path):
        ...
        try:
            ...
            raise FileNotFoundError("JSON file " + path + " not found!")
    ...
    def _make_edges(self):
        for id, parent_node in self._nodes.items():
            if "@ref" in parent_node.attributes.keys():
                for child_node_id in parent_node.attributes['@ref']:
                    child_node = self._nodes[child_node_id]
                    edge = Edge(parent_node, child_node, False)
                    self._graph.edges.append(edge)
                del parent_node.attributes['@ref']
